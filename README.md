...
#### DCML Pre-processing
From a DCML annotation corpus, you must first create aggregated tsv data with the `aggregate_corpus_data.py` script:
...
* A pre-created version of the F-H data (with default splits and seed) is in the h5_data-roman_xml directory.
...
### Training
Pre-trained models for the internal data can be found in the checkpoints-grid_search (all trained models) and checkpoints-grid_search-best (best performing models) directories.  
Pre-trained models for the Functional Harmony corpus can be found in the checkpoints-roman_xml-grid_search (all trained models) and checkpoints-roman_xml-best (best performing models) directories.
...
The json files used for grid search for the results in the paper are in the model_jsons directory.
