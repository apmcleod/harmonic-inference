...
def get_conf_mat(labels, outputs, normalize=True):
    """
...
def print_result(index, labels, outputs, limit=None, prob=True):
    """
    ...
    print('\n'.join([f'{label_string}: {output}'
                     for output, label_string in sorted(zip(outputs_this, label_strings), reverse=True)[:limit]]))
    
    
    
def get_input_df_rows(index, dataset):
    """
...
def get_correct_ranks(labels, outputs):
    """
...
def get_eval_df(labels, outputs, dataset):
    """
    ...
    if contains_dfs:
        ...
        for i in range(len(dataset)):
            chord, onset_notes, all_notes = get_input_df_rows(i, dataset)
