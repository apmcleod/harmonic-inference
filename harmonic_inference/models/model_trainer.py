"""Code to train (or evaluate) a model."""
import os
import sys
import shutil
...
from harmonic_inference.data.harmonic_inference_data import pad_and_collate_samples
...
class ModelTrainer():
    ...
    def __init__(self, model, train_dataset=None, valid_dataset=None, test_dataset=None,
        ...
        early_stopping : int
            Stop before num_epochs if there has been no improvement in validation loss for this
            many epochs.
        ...
        self.train_loader = (DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                                        collate_fn=pad_and_collate_samples)
        ...
        if resume is not None:
            ...
            if self.scheduler is not None:
                if 'scheduler' in checkpoint:
                    ...
                    print("Resume file has no scheduler information. Not loading it.",
                          file=sys.stderr)
            elif 'scheduler' in checkpoint:
                print("Resume file has scheduler information saved, but scheduler is None. "
                      "Not loading it.", file=sys.stderr)
    ...
    def train(self):
        ...
        for self.epoch in range(self.epoch, self.num_epochs):
            # Training
            train_loss, train_acc, _, _ = self.iteration(train=True)

            # Validation
            valid_loss, valid_acc, _, _ = self.iteration(train=False)
    ...
    def iteration(self, train=False):
        """
        ...
        ----------
        train : bool
            If True, use self.train_loader and perform backprop. If False, use self.valid_loader
            and perform no backprop.
        """
        total_loss = 0
        total_acc = 0
        total_size = 0
    ...
    def save(self, is_best=False, valid_acc=None):
        ...
        ----------
        is_best : bool
            If True, this copies the resulting checkpoint file to best.pth.tar in the same
            directory.
